...
# Using this option triggers a heap dump in the event of an out-of-memory condition
# The heap dump file consists of references to objects that cause the heap to overflow.
# By default, Cassandra puts the file a subdirectory of the working, root directory when running as a service.
#       - If Cassandra does not have write permission to the root directory
#         the heap dump fails
#       - If the root directory is too small to accommodate the heap dump
#         the server crashes
# 1. Accessible to Cassandra for writing
# 2. Large enough to accommodate a heap dump
